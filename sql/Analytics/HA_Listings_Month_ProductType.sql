...
select					
	year(txn.postdate_r) as Year,	month(txn.postdate_r) Month ,		
	txn.PlatformId ,	HAProd.ProductType,		
	count(distinct(c.accountId)) as ListingsCount 
from					
	YapstoneDM.dbo.[Transaction] txn				
	join YapstoneDM.dbo.Company c on txn.PlatformId = c.PlatformId and txn.Ref_CompanyId = c.CompanyId	
