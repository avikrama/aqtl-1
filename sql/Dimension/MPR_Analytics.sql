...
declare @date as date = '2015-12-31',

@COGS_Total as int = 7824984,/* @COGS_MPR as int = 7474458*/ @COGS_MPR as decimal(25,2), @Card_Volume_USD_Allocable as decimal(25,2) , 
@Homeaway_Domestic as int = 3301991, @Homeaway_Intl as int = 200000, @NonProfit as int = 73830 , @Known_COGS as nvarchar(max),
...
select
	...
	sum(cast(Revenue_Net_USD as decimal(25,2)))/sum(cast(TPV_Net_USD as decimal(25,2)))*100 Revenue_Rate,
	sum(cast(COGS_USD as decimal(25,2)))/sum(cast(TPV_Net_USD as decimal(25,2)))*100 COGS_Rate,
	(sum(Revenue_Net_USD)-sum(COGS_USD))/sum(TPV_Net_USD)*100  Profit_Rate,
	(sum(Revenue_Net_USD)-sum(COGS_USD))/sum(Revenue_Net_USD)*100 Accounting_Margin 
from 
	#MPR MPR
group by
	Vertical
