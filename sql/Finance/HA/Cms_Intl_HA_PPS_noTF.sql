...
select * from (
	select Software, Date, Currency+''_''+col col, Value from (
		select Software, Date, Currency, Txn_Amount, Revenue_after_TF, cast(Txn_Count as decimal(38,4)) Txn_Count
		from #Report
		where ProductType in (''PPS'')
	) src
	unpivot (
		value
		for col in (Txn_Amount, Revenue_after_TF, Txn_Count)
	) up
) src
pivot (
	sum(value)
	...
	select col from (
		...
		from #Report
			cross apply ( 
				  select 'Txn_Count'  col,		1 src_Sort_Order union all
				  select 'Txn_Amount' ,			2 union all 
				  select 'HA_Commission' ,		3 union all
				  --select 'Volume_after_TF',		3 union all 
				  --select 'Revenue',				4 union all 
				  select 'Revenue_after_TF',	4 
			) c
		) src
	order by Currency, src_Sort_Order
for xml path (''), type).value('.', 'nvarchar(max)'),1,1,'')
...
select * from (
	select Software, Date, Currency+''_''+col col, Value from (
		select Software, Date, Currency, cast(Txn_Count as decimal(38,4)) Txn_Count, Txn_Amount, HA_Commission, Revenue_after_TF	--Volume_after_TF, Revenue, Revenue_after_TF , HA_Commission
		from #Report
		where ProductType in (''PPB'')
	) src
	unpivot (
		value
		for col in ( Txn_Count, Txn_Amount, HA_Commission, Revenue_after_TF)				-- Volume_after_TF, Revenue, Revenue_after_TF, HA_Commission)
	) up
) src
pivot (
	sum(value)
