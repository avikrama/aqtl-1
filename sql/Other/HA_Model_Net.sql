declare @now as date, @start as date, @end as date, @start_new as date, @pricing_change as date = '2015-12-22'
 
set @now = getdate()
set @start = dateadd(mm,(year(@now)- 1900) * 12 + month(@now) - 1 -1 , 0)
set @end = dateadd(d,-1 , dateadd(mm,(year(@now)- 1900) * 12 + month(@now)- 1 , 0)) 
 
if object_id('tempdb..#Discount_Pricing_Old') is not null drop table #Discount_Pricing_Old
select * into #Discount_Pricing_Old from (
       select
               c.Platformid, c.PropertyOwnerAccountId --, c.PropertyOwnerName , sum(txn.Amount) TPV, sum(txn.AmtNetPropFee) Revenue, sum(txn.AmtNetPropFee)/sum(txn.Amount)*100 as '%'
       from
       ...
       where
               txn.TransactionCycleId in (1) and txn.PlatformId in (3) and txn.PaymentTypeId not in (4,5) and pt.ProductType in ('PPS')
               and txn.PostDate_R between @start and @pricing_change
               and ( ( txn.Amount > 200 ) and
               ( ( txn.AmtNetPropFee  / txn.Amount ) * 100 ) < 2.40
               )
       group by
               c.Platformid, PropertyOwnerAccountId--,c.PropertyOwnerName
       ) src
 
if object_id('tempdb..#Discount_Pricing_New') is not null drop table #Discount_Pricing_New
select * into #Discount_Pricing_New from (
       select
               c.Platformid, c.PropertyOwnerAccountId--, c.PropertyOwnerName , sum(txn.Amount) TPV, sum(txn.AmtNetPropFee) Revenue, sum(txn.AmtNetPropFee)/sum(txn.Amount)*100 as '%'
       from
               YapstoneDM..[Transaction] txn
       ...
       where
               txn.TransactionCycleId in (1) and txn.PlatformId in (3) and txn.PaymentTypeId not in (4,5) and pt.ProductType in ('PPS')
               and txn.PostDate_R between @pricing_change and @end
               and ( ( txn.Amount > 200 ) and
...
select * into #Discount_Pricing from (
       select * from #Discount_Pricing_Old union
       select * from #Discount_Pricing_New
) src
...
select year(txn.PostDate_R) Year , month(txn.PostDate_R) Month , cast(dateadd(d, -1 , dateadd(mm, (year(txn.PostDate_R) - 1900) * 12 + month(txn.PostDate_R) , 0)) as date) Date ,
       ...
       case when c2.PlatformId is not null then 'DiscountPricing' else Product.ProductType end ProductType ,
...
from                                          
   ...
   left join ETLStaging..FinanceHAPropertyOwners c1 on c1.Platformid = txn.PlatformId and c1.ChildCompanyId = txn.Ref_CompanyId
   left join #Discount_Pricing c2 on c1.Platformid = c2.PlatformId and c1.PropertyOwnerAccountId = c2.PropertyOwnerAccountId
...
group by year(txn.PostDate_R) , month(txn.PostDate_R) , cast(dateadd(d, -1 , dateadd(mm, (year(txn.PostDate_R) - 1900) * 12 + month(txn.PostDate_R) , 0)) as date), 
               case when c2.PlatformId is not null then 'DiscountPricing' else Product.ProductType end ,
